...
document.querySelector('#location-form').addEventListener('submit', async (event) => {
  ...
  const body = {
    ...
    available: document.querySelector('#available').value === 'true'
  };
...
document.querySelector('#load-demo').addEventListener('click', async () => {
  const demos = [
    ['driver-auto', { latitude: -27.4693, longitude: -58.8307, vehicleType: 'AUTO', available: true }],
    ['driver-moto', { latitude: -27.4701, longitude: -58.8312, vehicleType: 'MOTO', available: true }]
  ];
  try {
...
document.querySelector('#search-form').addEventListener('submit', async (event) => {
  ...
  const params = new URLSearchParams({
    latitude: document.querySelector('#search-latitude').value,
    longitude: document.querySelector('#search-longitude').value,
    vehicleType: document.querySelector('#search-vehicle').value,
    radiusKm: document.querySelector('#radius').value,
    maxCandidates: '10'
  });
  try {
    ...
    for (const driver of data.drivers) {
      ...
      const title = document.createElement('strong'); title.textContent = driver.driverId;
      const detail = document.createElement('p'); detail.textContent = `${driver.vehicleType} · ${driver.distanceKm} km · ETA ${driver.estimatedEtaMinutes} min`;
      ...
      const toggle = document.createElement('button'); toggle.className = 'secondary'; toggle.textContent = 'Marcar no disponible';
      toggle.addEventListener('click', async () => { try { await callApi(`${api}/drivers/${encodeURIComponent(driver.driverId)}/availability`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: '{"available":false}' }); row.remove(); } catch {} });
      const remove = document.createElement('button'); remove.className = 'secondary'; remove.textContent = 'Eliminar ubicacion';
      remove.addEventListener('click', async () => { try { await callApi(`${api}/drivers/${encodeURIComponent(driver.driverId)}/location`, { method: 'DELETE' }); row.remove(); } catch {} });
